...
def tt_optimize(G, dims, I_data, I_data_std, sigma=1e-5):
    ...
    def eval_r(x):
        ...
        if sigma is None: # no regularization
            ...
            reg_rp = sigma * cp.diff(w_rp)
            reg_re = sigma * cp.diff(w_re)
            reg_theta = sigma * cp.diff(w_theta)
            reg_phi = sigma * cp.diff(w_phi)
            return cp.hstack((eps.flatten(),reg_rp,reg_re,reg_theta,reg_phi)).get()
    ...
    def eval_Jr(x):
        ...
        # w_rp derivative
        Gw_dwrp = cp.tensordot(cp.tensordot(cp.tensordot(G, w_re, axes=(3,0)), w_theta, axes=(3,0)), w_phi, axes=(3,0))
        dwrp = ( xi * Gw_dwrp ) / I_data_std[:,:,cp.newaxis]

        # w_re derivative
        Gw_dwre = cp.tensordot(cp.tensordot(cp.tensordot(G, w_rp, axes=(2,0)), w_theta, axes=(3,0)), w_phi, axes=(3,0))
        dwre = ( xi * Gw_dwre ) / I_data_std[:,:,cp.newaxis]

        # w_theta derivative
        Gw_dwt = cp.tensordot(cp.tensordot(cp.tensordot(G, w_rp, axes=(2,0)), w_re, axes=(2,0)), w_phi, axes=(3,0))
        dwt = ( xi * Gw_dwt ) / I_data_std[:,:,cp.newaxis]

        # w_phi derivative
        Gw_dwp = cp.tensordot(cp.tensordot(cp.tensordot(G, w_rp, axes=(2,0)), w_re, axes=(2,0)), w_theta, axes=(2,0))
        ...
        deps = cp.hstack((dxi[:,cp.newaxis],db[:,cp.newaxis],dwrp,dwre,dwt,dwp)).flatten()

        # handle regularization
        if sigma is None: # no regularization
            return deps.get()
        else: # regularization term derivatives (sparse)
            dreg_rp1 = sigma * cp.ones(nrp-1)
            dreg_rp2 = -sigma * cp.ones(nrp-1)
            dreg_re1 = sigma * cp.ones(nre-1)
            dreg_re2 = -sigma * cp.ones(nre-1)
            dreg_t1 = sigma * cp.ones(ntheta-1)
            dreg_t2 = -sigma * cp.ones(ntheta-1)
            dreg_p1 = sigma * cp.ones(nphi-1)
            dreg_p2 = -sigma * cp.ones(nphi-1)
            return cp.hstack((deps,dreg_rp1,dreg_rp2,dreg_re1,dreg_re2,dreg_t1,dreg_t2,dreg_p1,dreg_p2)).get()
    ...
    x0_scaled = cp.hstack((xi0/xi_sc,b0/b_sc,w_rp_0,w_re_0,w_theta_0,w_phi_0)).get()
