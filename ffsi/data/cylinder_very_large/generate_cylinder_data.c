/*
 * Generate Intensity Data and Distributions for the SAS Cylinder
 * (takes about four hours on 16 cores for the very large example)
 *
 * Compile: gcc -fopenmp -O2 generate_cylinder_data.c -o gen_cyl -lm
 */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
double G_cylinder(double qx, double qy, double l, double r, double theta, double phi, double drho) {
    ...
    double hqcl = 0.5 * qc * l;
    double qar = qa * r;
    double F = 2.0 * V * drho * (sin(hqcl) / hqcl) * (j1(qar) / qar);
...
int main() {
    ...
    double *l = (double *)malloc(nl * sizeof(double));
    ...
    linspace((double)ll, (double)lu, nl, l);
    linspace((double)rl, (double)ru, nr, r);
    linspace((double)thetal, (double)thetau, ntheta, theta);
    linspace((double)phil, (double)phiu, nphi, phi);

    double *w_l_true = (double *)malloc(nl * sizeof(double));
    double *w_r_true = (double *)malloc(nr * sizeof(double));
    ...
    int ng = 3;
    Gaussian gl[] = {{1.5, 300, 20}, {1, 400, 20}, {2, 500, 20}};
    crazy_distribution(l, nl, gl, ng, 0, 1, 1, w_l_true);
    Gaussian gr[] = {{1, 60, 3}, {2, 70, 4}, {2, 80, 3}};
    crazy_distribution(r, nr, gr, ng, 0, 1, 1, w_r_true);
    ...
    // save distributions
    FILE *fl = fopen("c_w_l_true.txt","w");
    for (int il = 0; il < nl; ++il) {
            fprintf(fl, "%.18e\n", w_l_true[il]);
    }
    fclose(fl);
    FILE *fr = fopen("c_w_r_true.txt","w");
    ...
    printf("b_true: %.2e\n", b_true);
    double *V = (double *)malloc((size_t)nl * nr * sizeof(double));
    for (int il = 0; il < nl; ++il) {
        for (int ir = 0; ir < nr; ++ir) {
            V[il * nr + ir] = M_PI * l[il] * r[ir] * r[ir];
        }
    }
    double V_ave = 0.0;
    for (int il = 0; il < nl; ++il) {
        for (int ir = 0; ir < nr; ++ir) {
            V_ave += w_l_true[il] * V[il * nr + ir] * w_r_true[ir];
        }
    }
    ...
    #pragma omp parallel for schedule(dynamic) default(none) shared(I_true, xi_true, b_true, qx, qy, l, r, theta, phi, w_l_true, w_r_true, w_theta_true, w_phi_true, nqx, nqy, nl, nr, ntheta, nphi, drho)
        ...
        for (int iqy = 0; iqy < nqy; ++iqy) {
            ...
            for (int il = 0; il < nl; ++il) {
                for (int ir = 0; ir < nr; ++ir) {
                    for (int it = 0; it < ntheta; ++it) {
                        for (int ip = 0; ip < nphi; ++ip) {
                            double g = G_cylinder(qx[iqx], qy[iqy], l[il], r[ir], theta[it], phi[ip], drho);
                            double wprod = w_l_true[il] * w_r_true[ir] * w_theta_true[it] * w_phi_true[ip];
