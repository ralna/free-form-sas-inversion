            ...
            for ir in range(nr):
                for it in range(ntheta):
                    for ip in range(nphi):
                        Gw_true[iqx,iqy] += G_cylinder(qx[iqx], qy[iqy], l[il], r[ir], theta[it], phi[ip], drho) \
                                            * w_l_true[il] * w_r_true[ir] * w_theta_true[it] * w_phi_true[ip]
...
print("\nStep 3: SAS inversion with low-rank G\n")
xi_opt, b_opt, w_l_opt, w_r_opt, w_theta_opt, w_phi_opt = tt_optimize(tt, dims, I_data, I_data, sigma=None,
