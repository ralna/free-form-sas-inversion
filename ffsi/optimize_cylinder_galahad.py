...
def tt_optimize(G, dims, I_data, I_data_std, sigma=None):
    ...
    G_ave = xp.sum(G, axis=(2,3,4,5)) / (nl*nr*ntheta*nphi)
    ...
    def eval_r(x):
        ...
        if sigma is None: # no regularization
            ...
            reg_l = sigma * xp.diff(w_l)
            reg_r = sigma * xp.diff(w_r)
            reg_theta = sigma * xp.diff(w_theta)
            reg_phi = sigma * xp.diff(w_phi)
            res = xp.hstack((eps.flatten(),reg_l,reg_r,reg_theta,reg_phi))
    ...
    def eval_Jr(x):
        ...
        # w_l derivative
        Gw_dwl = xp.tensordot(xp.tensordot(xp.tensordot(G, w_r, axes=(3,0)), w_theta, axes=(3,0)), w_phi, axes=(3,0))
        dwl = ( xi * Gw_dwl ) / I_data_std[:,:,xp.newaxis]

        # w_r derivative
        Gw_dwr = xp.tensordot(xp.tensordot(xp.tensordot(G, w_l, axes=(2,0)), w_theta, axes=(3,0)), w_phi, axes=(3,0))
        dwr = ( xi * Gw_dwr ) / I_data_std[:,:,xp.newaxis]

        # w_theta derivative
        Gw_dwt = xp.tensordot(xp.tensordot(xp.tensordot(G, w_l, axes=(2,0)), w_r, axes=(2,0)), w_phi, axes=(3,0))
        dwt = ( xi * Gw_dwt ) / I_data_std[:,:,xp.newaxis]

        # w_phi derivative
        Gw_dwp = xp.tensordot(xp.tensordot(xp.tensordot(G, w_l, axes=(2,0)), w_r, axes=(2,0)), w_theta, axes=(2,0))
        ...
        deps = xp.hstack((dxi[:,xp.newaxis],db[:,xp.newaxis],dwl,dwr,dwt,dwp)).flatten()

        # handle regularization
        if sigma is None: # no regularization
            jac = deps
        else: # regularization term derivatives (sparse)
            dreg_l1 = sigma * xp.ones(nl-1)
            dreg_l2 = -sigma * xp.ones(nl-1)
            dreg_r1 = sigma * xp.ones(nr-1)
            dreg_r2 = -sigma * xp.ones(nr-1)
            dreg_t1 = sigma * xp.ones(ntheta-1)
            dreg_t2 = -sigma * xp.ones(ntheta-1)
            dreg_p1 = sigma * xp.ones(nphi-1)
            dreg_p2 = -sigma * xp.ones(nphi-1)
            jac = xp.hstack((deps,dreg_l1,dreg_l2,dreg_r1,dreg_r2,dreg_t1,dreg_t2,dreg_p1,dreg_p2))
    ...
    x0_scaled = xp.hstack((xi0/xi_sc,b0/b_sc,w_l_0,w_r_0,w_theta_0,w_phi_0))
