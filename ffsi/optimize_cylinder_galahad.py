...
def optimize(G, I_data, I_data_std, sigma=None):
    ...
    w0_list = [xp.ones(n) / n for n in dims[2:]]

    # this averages out G over the parameters
    # TODO: this is special case
    G_ave = xp.sum(G, axis=tuple(range(2,G.ndim))) / np.prod(dims[2:])
    ...
    def eval_r(x):
        ...
        xi = x[0] * xi_sc
        b = x[1] * b_sc
        split_inds = xp.cumsum(dims[2:])[:-1]
        w_list = xp.split(x[2:], split_inds)

        # form Gw (the form factor)
        Gw = contract_tensor(G, w_list, skip_axes=[0,1])
    ...
    def eval_Jr(x):
        ...
        for i in range(len(dims[2:])):
            ...
            Gw_dw = contract_tensor(G, w_contract_list, skip_axes=[0,1,2+i])
        ...
        if sigma is None: # no regularization
            ...
            dreg1 = [sigma * xp.ones(n-1) for n in dims[2:]]  # w[i+1] terms
            dreg2 = [-sigma * xp.ones(n-1) for n in dims[2:]] # -w[i] terms
    ...
    if sigma is None: # no regularization
        ...
        m_r = np.prod(dims[:2]) + np.sum(np.array(dims[2:])-1)
    m_c = len(dims[2:])

    # set GALAHAD SNLS cohorts
    ch_list = [i * np.ones(n, dtype=int) for i,n in enumerate(dims[2:])]
    ...
    if sigma is None: # no regularization
        ...
        nq = np.prod(dims[:2])
        Jr_ne = nq*n + 2*np.sum(np.array(dims[2:])-1)
        # flattened intensity misfit derivative
        Jr_eps_row = np.tile(np.arange(nq),(n,1)).flatten('F')
        Jr_eps_col = np.tile(np.arange(n),nq)
        # sparse regularization derivatives for w
        split_inds = np.cumsum(np.array(dims[2:])-1)[:-1]
        Jr_reg1_row = np.split(np.arange(nq,nq+np.sum(np.array(dims[2:])-1)), split_inds)
        Jr_reg2_row = Jr_reg1_row.copy()
        Jr_reg1_col = [] # w[i+1] terms
        Jr_reg2_col = [] # -w[i] terms
        starts = np.cumsum(dims[2:]) + 2 - dims[2:]
        for st, dim in zip(starts, dims[2:]):
